...
public class LazyStreamTextSource extends Source<CharSequence> {
	...
	public LazyStreamTextSource(InputStream inputStream) {
		this(inputStream, -1);
	}

	public LazyStreamTextSource(InputStream inputStream, int lazyLoadBufferSize) {
	...
	protected CharSequence onOpen(LoadingStrategy strategy) throws SourceOpenException {
		if (strategy == LoadingStrategy.LOAD_PREVIOUS) {
			...
			/* noop */
		} else if (strategy == LoadingStrategy.INIT) {
			mIndex = -1;
		} else {

...
public class MixTask implements TaskQueue.Listener<MixTask.Args, TypesetEngine.TypesetResult>, TaskQueue.Task<MixTask.Args, TypesetEngine.TypesetResult> {
    ...
    private void typesetDocument(TaskQueue.Token token, final int outWidth, Document document, RenderOption option, TexasView.SegmentDecoration segmentDecoration) throws TaskQueue.TokenExpiredException {
        int size = document.getSegmentCount();

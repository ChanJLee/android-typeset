...
public class BookParser extends TexasView.Adapter<CharSequence> {
	...
	public BookParser(android.content.Context context, TexasView texasView, int policy) {
		...
		mFlagWidth = resources.getDimension(me.chan.androidtex.R.dimen.com_shanbay_lib_texas_flag_width);
		mFlagHeight = resources.getDimension(me.chan.androidtex.R.dimen.com_shanbay_lib_texas_flag_height);
		// for test
	...
	private void setupUserDefineView(List<Segment> segments) {
		// 添加自定义的视图
		segments.add(new ViewSegment(me.chan.androidtex.R.layout.test_header) {
	...
	private void setupIncrementalUserDefineView(List<Segment> segments) {
		segments.add(new ViewSegment(me.chan.androidtex.R.layout.test_layout, true) {
			@Override
			protected void onRender(View view) {
				if (view.getTag() != null) {
				...
				view.findViewById(me.chan.androidtex.R.id.finish).setOnClickListener(new View.OnClickListener() {
					@Override
		...
		segments.add(new ViewSegment(me.chan.androidtex.R.layout.test_layout2, true) {
			@Override
			protected void onRender(View view) {
	...
	private List<Segment> parseArticleContent(XmlPullParser parser, TexasOption texasOption) throws IOException, XmlPullParserException {
		...
		// 测试页面滚动
		segments.add(new ViewSegment(me.chan.androidtex.R.layout.test_layout) {
